...
const fields = { date: "Date", description: "Description / reference", amount: "Amount", debit: "Debit / money out",
  credit: "Credit / money in", currency: "Currency", category: "Category", balance: "Statement balance", transaction_type: "Transaction type" };
...
export default function UploadPage() {
  ...
  const applyDetection = (data) => {
    setDetected(data);
    setOptions((old) => ({ ...old, sheet: data.sheet, header_row: data.header_row,
  ...
  const upload = (file) => {
    ...
    act("Reading your file…", async () => {
      ...
      setFileName(file.name); applyDetection(data);
    });
  };
  ...
  return <div className="ft-app">
    ...
    <main className="ft-main ft-import">
      ...
      {detected && <fieldset disabled={!!busy} className="ft-import-fieldset">
        <section className="ft-panel">
          <h2>2. Check the columns</h2><p className="ft-muted">{fileName} · {detected.row_count} source rows in {detected.sheet}. Detection is a suggestion; your confirmation is required.</p>
          ...
          <div className="ft-import-grid">{Object.entries(fields).map(([key, label]) => <label key={key}>{label}<span className="ft-muted">Suggestion: {detected.mapping[key].confidence}</span>
          ...
          <p className="ft-muted">Map Date and Description, plus either Amount or Debit/Credit. Optional fields stay unmapped when uncertain. Use the Type column or row overrides for transfers.</p>
        ...
        <section className="ft-panel"><h2>3. Set how to read the values</h2><div className="ft-import-grid">
          ...
          <label>Signed amount convention<select value={options.sign_convention} onChange={(e) => update({ sign_convention: e.target.value })}><option value="negative_expense">Negative = expense; positive = income</option><option value="positive_expense">Positive = expense; negative = income</option></select></label>
        ...
        {review && <section className="ft-panel" aria-label="Validated transactions"><h2>4. Review before importing</h2>
          ...
          <p className="ft-muted">{review.warning} Possible duplicates are skipped. Identical legitimate payments may need distinct source references.</p>
          <div className="ft-table-scroll"><table><caption>Normalized preview · rows {page * 10 + 1}–{Math.min((page + 1) * 10, review.rows.length)} of {review.rows.length}</caption>
            <thead><tr><th>Source row</th><th>Date</th><th>Description</th><th>Amount</th><th>Type</th><th>Category</th><th>Status / errors</th><th>Exclude</th></tr></thead>
            <tbody>{review.rows.slice(page * 10, page * 10 + 10).map((row) => <tr key={row.source_row}><td>{row.source_row}</td><td>{row.transaction?.date.slice(0, 10) || "—"}</td><td>{row.transaction?.description || "See source row"}</td><td>{row.transaction ? cash(row.transaction.amount) : "—"}</td><td>
          ...
          <button disabled={!confirmed || review.stale || (!!review.invalid && !skipInvalid) || !review.to_import} onClick={finish}>Confirm import</button>
        </section>}
      </fieldset>}
