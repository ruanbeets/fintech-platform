...
- Signed amounts or separate Debit/Credit normalize to positive amounts plus income/expense type. A recognized source Type overrides sign classification. Row overrides let the reviewer explicitly identify transfers, which existing analytics exclude.
...
- Source balances are provenance only. Account balances and net worth remain unavailable.
- Fingerprint: session + normalized account label + date/time + whitespace/case-normalized description + exact amount + currency + type. Same-file and prior-import matches are shown and skipped. Distinct identical legitimate transactions can be false duplicates; bank-specific reference reconciliation is outside this slice.
